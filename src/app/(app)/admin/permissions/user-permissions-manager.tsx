...
/** Cấp quyền riêng cho ĐÚNG 1 người dùng — chỉ mở thêm 1 vài trang cụ thể
 * cho họ mà không cần mở cho cả vai trò (vd 1 Saler cụ thể được xem thêm
 * /ads-cost). Chọn người ở trên, sửa lưới bên dưới, Lưu chỉ gửi đúng người
 * đang chọn — khác PermissionsManager (gửi cả lô mọi vai trò cùng lúc) vì ở
 * đây luôn chỉ có 1 người đang được sửa tại một thời điểm. */
        ...
        <div className="flex flex-col gap-8">
          {PERMISSION_FEATURE_GROUPS.map(({ group, features }) => (
            <div key={group}>
              ...
              <UserPermissionsTable features={features} grants={currentGrants} onToggle={toggle} />
            </div>
          ))}
        </div>
      )}
