...
export function PushFollowupDialog({
  ...
  onConfirm: (suggestion: string) => Promise<void>;
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      await onConfirm(suggestion.trim());
      onOpenChange(false);
    } catch (err) {
      ...
      <DialogContent className="shadow-bubble overflow-hidden rounded-2xl border border-border/70 bg-card/98 p-5 backdrop-blur-xl sm:max-w-xl sm:p-7">
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <DialogHeader className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 pr-8">
            ...
            <DialogTitle className="text-lg">Gửi yêu cầu chăm sóc lại ({selectedCount})</DialogTitle>
            <DialogDescription>
              Yêu cầu sẽ vào hàng đợi chờ Leader phân bổ Sale phụ trách — bạn không cần chọn Sale ở đây.
            </DialogDescription>
          </DialogHeader>

          <div className="flex flex-col gap-1.5">
            <Label htmlFor="followup-suggestion">Ghi chú cho Leader (tuỳ chọn)</Label>
            <Textarea
              id="followup-suggestion"
              value={suggestion}
              onChange={(e) => setSuggestion(e.target.value)}
              placeholder="Vd: khách hỏi về học phí, gọi lại buổi chiều... — Leader sẽ thấy ghi chú này khi phân bổ, và Sale được gán cũng sẽ thấy lại."
