...
/** Dùng chung cho cả gán 1 khách lẫn gán hàng loạt — mirror
 * assign-followup-dialog.tsx nhưng nội dung dành cho tư vấn ghi danh. */
      ...
      <DialogContent className="shadow-bubble overflow-hidden rounded-2xl border border-border/70 bg-card/98 p-5 backdrop-blur-xl sm:max-w-md sm:p-7">
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <DialogHeader className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 pr-8">
            ...
            <DialogTitle className="text-lg">Phân bổ {selectedCount} khách hàng cho Sale</DialogTitle>
            <DialogDescription>Sale được chọn sẽ thấy khách hàng này ở Workspace của họ và bắt đầu tư vấn ghi danh.</DialogDescription>
          </DialogHeader>

          <div className="flex flex-col gap-1.5 rounded-2xl border border-border/60 bg-secondary/30 p-4">
            <Label htmlFor="assign-customer-target">Sale phụ trách</Label>
            <Select value={targetSaleEmail} onValueChange={(v) => setTargetSaleEmail(v ?? "")} items={salesItems} disabled={loadingSales}>
              <SelectTrigger id="assign-customer-target" className="h-11 w-full rounded-xl bg-background">
